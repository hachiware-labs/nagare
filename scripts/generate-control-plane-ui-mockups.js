...
function domainQualitySettings() {
  ...
  function agentChip(x, y, name, role, kind = "green") {
    ...
    ${txt(x + 16, y + 44, "この成果物の依頼で候補に入る", "small")}`;
  }
  ...
  const content = `
    ...
    ${txt(276, 190, "ドメインごとの共通基準と、成果物ごとのレビュー観点を管理します。", "body")}
    ...
    ${txt(276, 790, "別ドメインの成果物はここには表示しません。", "small", C.muted)}
    ...
    ${txt(1132, 316, "この成果物を使うエージェント", "small")}
  ...
  return chrome(
    "artifact-types",
    "ドメイン",
    "ドメインと成果物ごとに品質基準を管理する",
    "",
    content,
    "ドメインに応じた成果物選択、選択成果物の品質基準、利用中エージェントと品質記録を確認する画面。"
  );
}
