...
use nagare_core::{
    AddAgentProfileInput, AgentProfile, AgentRunPurpose, LocaleSettings, NagareAgentSettings,
    RuleResolution, RunWorkItemInput, SetLocaleInput, SetNagareAgentSettingsInput, VERSION,
    WorkItemSnapshot, add_agent_profile, agent_doctor, agent_probe, approve_work_item,
    ...
    list_work_items, resolve_root, resolve_rule_for_path, run_first_scenario,
...
fn run_item_with_nagare_agent(
    ...
    .map_err(|e| e.to_string())?;
    println!(
        "run {} {} purpose={} agent={} exit={:?} evidence={} item_status={}",
        result.run.id,
        result.run.status,
        result.run.purpose,
        result.run.agent_profile_id,
...
fn print_scenario_result(label: &str, result: &nagare_core::ScenarioResult) {
    println!("{label}");
...
fn print_snapshot(snapshot: &WorkItemSnapshot) {
    println!(
        "{}\t{}\t{}",
...
fn print_agent_profile_row(profile: &AgentProfile) {
    println!(
        "{}\t{}\t{}\t{}\t{}\t{}",
        profile.id,
        profile.adapter,
...
fn print_agent_defaults(settings: &NagareAgentSettings) {
...
fn print_rule_resolution(resolution: &RuleResolution) {
...
fn dispatch_prompt(resolution: &RuleResolution) -> String {
    format!(
        "Prepare a dispatch preview for path `{}`.\nMatched rule: {}\nTarget agent profile: {}\nReview agent profile: {}\nSkill sets: {}\nPermission policy: {}\nWorkspace policy: {}\nVerification: {}\nReturn the recommended execution plan, risks, and any missing information.",
...
fn print_locale_settings(settings: &LocaleSettings) {
    println!("language: {}", settings.language);
    println!("timezone: {}", settings.timezone);
}

fn print_created(label: &str, created: bool, path: &std::path::Path) {
    if created {
