...
| --- | --- | --- | --- | --- | --- |
| 1.1.1 | Project を初期化すると、`.nagare` 配下に設定、台帳、Domain / Artifact Type / Agent / artifact / log 保存先を作成する。 | project root が存在する | `nagare init` を実行する | `.nagare/project.toml`、`.nagare/domains/general.toml`、`.nagare/artifact-types/general.toml`、`.nagare/agents/`、`.nagare/state/ledger.json`、`.nagare/artifacts/`、`.nagare/logs/` が存在する | 実装済み |
...
| 2.1.9 | Work Item に workflow mode を保存できる。 | Project が初期化済み | `nagare item create --workflow-mode confirm_first\|finish_first` を実行する | Work Item snapshot に `workflow_mode` が保存され、未指定時は Domain override、Domain default、Project workflow default の順に解決される | 実装済み |
| 2.1.10 | Work Item に Domain、Artifact Type、最終承認ポリシーを固定できる。 | Project が初期化済み | `nagare item create --domain <domain_id> --artifact-type <artifact_type_id> --approval-policy manual_final_approval\|auto_complete_on_review_pass` または Settings の Work Item 作成フォームを使う | Work Item snapshot に `domain_id` / `artifact_type_id` と解決済み `approval_policy` が保存され、Artifact Type が所属する Domain と矛盾する Domain 指定を拒否する | 実装済み |
...
| 3.1.2 | Agent Profile には表示名、role、runtime、adapter、working_dir、description、specialties、担当 Domain / Artifact Type を保存できる。 | Project が初期化済み | `nagare agent add` に各 option を渡す、または Settings の Agent form を送信する | 保存された TOML に値が残り、dispatch prompt の候補 summary に反映される | 実装済み |
...
| 3.1.5 | Project-local Artifact Type を登録できる。 | Project が初期化済み | Settings の Artifact Type 作成フォームを送信する | `.nagare/artifact-types/<id>.toml` が作成され、`domain_id`、description、artifact_types、rubric、dispatch_hints、workflow override が保存される | 実装済み |
| 3.1.6 | Artifact Type を後から更新できる。 | Artifact Type が存在する | Settings の Artifact Type 編集フォームを送信する | `.nagare/artifact-types/<id>.toml` が更新され、Settings の Artifact Types 一覧に反映される | 実装済み |
| 3.1.7 | Domain は大きな業務領域を束ねる第一級概念として扱う。 | 複数Artifact Typeが同じ業務領域に属する | Settings の Domain 作成フォームを送信する | `.nagare/domains/<id>.toml` に共通知識、共通Rubric、dispatch_hints、workflow default が保存され、Artifact Type は `domain_id` で所属を持てる | 実装済み |
...
| 3.2.4 | Settings の Agent 一覧を Domain / Artifact Type で絞り込める。 | Domain / Artifact Type と Agent Profile が存在する | Settings の Agents で Domain または Artifact Type の checkbox を選ぶ | 選択した Domain または Artifact Type に属する Agent Profile だけが表示され、Clear filters で全件表示に戻る | 実装済み |
...
| 3.2.10 | Settings の Agent 一覧は Agent を中心に Tool / Model / Skills / Scope / Actions を確認できる。 | Agent Profile が存在する | Settings の Agents を開く | Agent 名、role、working_dir、tool kind、model、source、skills、Domain / Artifact Type scope が横断的に確認でき、モバイルでは card-like row として読める | 実装済み |
...
| 3A.1.3 | Domain は Project workflow を上書きできる。 | Artifact Type が存在する | Domain 編集フォームで workflow override を保存する | Work Item 作成時に domain を選ぶと、Domain override が Project default より優先される | 実装済み |

## 4. Agent Selection / Dispatch 準備

| ID | 仕様 | Given | When | Done | 状態 |
| --- | --- | --- | --- | --- | --- |
| 4.1.1 | Agent Profile、Domain、Artifact Type は選択材料を持つ。 | Agent / Domain / Artifact Type を登録する | Agent の role、specialties、description、working_dir、Domain / Artifact Type scope と、Domain / Artifact Type の rubric、dispatch_hints を設定する | dispatch が候補比較に使える情報が `.nagare/agents/<id>.toml`、`.nagare/domains/<id>.toml`、`.nagare/artifact-types/<id>.toml` に残る | 実装済み |
| 4.1.1a | Domain はdispatchの選択材料として使う。 | Domain が定義されている | dispatch context を組み立てる | Domain の共通知識、共通Rubric、dispatch_hints、workflow default が Artifact Type と合わせて候補比較に使える | 実装済み |
...
| 4.2.10 | Agent Profile は Domain × Artifact Type × role で分化できる。 | `code-planner`、`docs-worker`、`ui-reviewer` のような Agent Profile が登録されている | dispatch preview を実行する | 候補 summary に `role`、`specialties`、`description`、`domain_ids`、`artifact_type_ids` が含まれ、dispatch_agent が Planner / Worker / Reviewer を Domain / Artifact Type に応じて選べる | 実装済み |
| 4.2.11 | 初期 Agent Profile は汎用 Domain に紐づく。 | Project が初期化済み | `nagare agent list` または Settings の Agent 一覧を見る | `worker`、`reviewer`、`dispatcher`、`supervisor` が `domain_ids = ["general"]`、`artifact_type_ids = ["general"]` を持つ | 実装済み |
...
| 16.1.18 | ローカルUIサーバの Settings で Workflow、Agent Profile、Artifact Type を確認できる。 | Project が初期化済み | `nagare ui serve` の Settings を開く | Agent Defaults form は表示せず、Project workflow default、登録済み Agent Profile、Artifact Type の rubric / dispatch_hints / workflow override を確認できる | 実装済み |
...
| 16.1.20a | ローカルUIサーバから Artifact Type を追加・編集できる。 | Project が初期化済み | Settings の Artifact Type 追加/編集フォームを送信する | `POST /api/artifact-types` または `POST /api/artifact-types/<id>` が `.nagare/artifact-types/<id>.toml` を作成・更新し、rubric、dispatch_hints、workflow override が保存される | 実装済み |
