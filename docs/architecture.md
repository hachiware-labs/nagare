...
作業フォルダ、Agent Profile の `working_dir`、CapabilityProbe、Verification 候補を表示する。
...
- `WorkItem`: id、title、description、acceptance_criteria、expected_artifacts、verification_hint、work_folder、constraints、locale、status、created_at、updated_at
...
- `ResolvedSkillContext`: id、work_item_id、agent_profile_id、capability_probe_id、capabilities_in_force、instruction_sources、artifact_uri、content_hash、locale
...
- `WorkItemTimelineEvent`: ledger の各recordから生成する read model。request、dispatch、run、artifact、evidence、question、human_feedback、review、verification、handoff、recovery、workflow_decision、decision をWork Itemの一本の流れとして表示する
...
- `HandoffPacket`: id、work_item_id、from_agent_profile、to_agent_profile、reason、summary、current_state、open_questions、artifact_ids、diff_artifact_ids、failed_verification_ids、review_result_ids、next_request、locale
- `HumanDecision`: id、work_item_id、decision_type、rationale、locale
- `ScopeResolution`: path、work_folder、agent_profile_id、review_agent_profile_id、scope_match、permission_policy_id、workspace_policy_id、verification、warnings

Agent Management 型:

- `RuntimeDeclaration`
- `AdapterDeclaration`
- `AgentProfileDeclaration`: runtime、adapter、role、working_dir、description、specialties、output_contracts を持つ
- `CapabilityProbe`
...
2. `nagare item preview` で Agent Profile、working_dir、CapabilityProbe、Policy、Verification、Run Packet を確認する。
...
2. `from_agent_profile`、`to_agent_profile`、reason、summary に加え、現在状態、未解決質問、artifact、diff、review、failed verification、next request を保存する。
...
- 整合性違反: passing verification なしの approval、Agent working_dir と work_folder の不整合
