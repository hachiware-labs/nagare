...
Agent Profile / Skill の詳細データ形式は `docs/agent_data_model.md`、
Adapter 境界は `docs/adapter_contract.md`、画面UIのデザイン方針は
`docs/design.md` を参照する。
...
- Core: Work Item、Agent Run、Evidence、Review、Handoff、Decision の不変条件と状態遷移を所有する。
- Agent Management: Runtime、Adapter、Agent Profile、working_dir、Capability Probe、Policy を解決する。
- Adapter Kernel: Run Packet を各 Agent runtime の入力形式へ変換し、Run Event / Output / ExecutionRecord / Evidence を回収する。
- Ledger: Work Item と実行証跡の永続化。MVP では JSON ledger、後続で SQLite を使う。
- Artifacts: ユーザーが依頼した成果物ファイル、またはその成果物を作るために必要な入力ファイル。
- Execution Records: log、raw output、diff、transcript などの実行記録。
...
| Domain | Work Item、Agent Run、Evidence、Review、Handoff、Decision、Run Packet の型、不変条件、状態遷移 | `crates/nagare-core/src/domain/*` | filesystem、process spawn、TOML/JSON I/O、CLI 出力を持たない |
| Agent Management | Runtime、Adapter、Agent Profile、working_dir、Policy、Capability Probe、Resolved Context の解決 | `crates/nagare-core/src/agent/*` | Work Item の完了判定や Human Decision を所有しない |
...
| Config Repository | `.nagare/project.toml` と `.nagare/agents/*.toml` を読む/書く | TOML files | Locale、Nagare Agent defaults、Agent Profile、Runtime |
| Ledger Repository | ledger-owned entity を load/save する | `ledger.json` | WorkItem、AgentRun、Evidence、Review、Handoff、Decision、Resolved records |
| Artifact Store | 成果物または成果物作成に必要な file を保存する | deliverable file | artifact URI |
| Execution Record Store | 実行ログ、raw output、diff、transcript などの証跡 file を保存する | bytes / structured record | execution record URI |
...
| Scope Resolver | 依頼時の work_folder と Agent Profile の working_dir / Probe から候補 Agent と実行前提を解決する | work_folder、optional agent override、Agent Profiles | ScopeResolution |
| Run Resolver | ScopeResolution と Probe から ResolvedSkillContext / ResolvedRunPacket を作る | Work Item、Agent Profile、ScopeResolution、Probe | ResolvedSkillContext / ResolvedRunPacket |
| Probe Resolver | Run / Preview 前に CapabilityProbe を再利用または更新する | Agent Profile、Runtime、既存 Probe | CapabilityProbe |
| Capability Resolver | Agent Profile の宣言属性と CapabilityProbe を照合し、dispatch candidate summary を作る | Agent Profile、CapabilityProbe | CapabilitySummary |
| Run Orchestrator | Agent Run を開始し、AgentOutputRecord / ExecutionRecord / Evidence / AgentRun を記録し、依頼された成果物だけ Artifact 化する | ResolvedRunPacket、prompt、adapter | RunWorkItemResult |
| Dispatch Planner | dispatch preview の Agent 出力を実行前確認用の DispatchPlan として保存する | AgentRun、ResolvedRunPacket、ExecutionRecord | DispatchPlan |
| Review Service | Review Agent の結果、CI/test/artifact check、指摘点を ReviewResult と Evidence にする | Work Item、review output | ReviewResult |
...
| Workflow Service | Work Item の次アクションを判断し、停止点まで推進する | Work Item Snapshot、Nagare Agent defaults、RecoveryPlan | WorkflowDecision / AdvanceWorkItemResult |
...
| Evidence Board | Core / Artifact Store | Artifact、Evidence、Review の関連付け |
| Agent Registry | Agent Management | Runtime、Adapter、Agent Profile、working_dir、Capability Probe の管理 |
...
- `ResolveWorkScope(root, work_folder, agent?) -> ScopeResolution`
- `PreviewWorkItemRun(root, work_item_id, work_folder?, agent?) -> RunPreview`
...
- smoke / CI / review check 用の local command
- 別言語の SDK wrapper
...
- `resolve_work_scope(root, work_folder?, agent?) -> ScopeResolution`
...
Agent Profile の `working_dir` は、AGENTS.md、skills、instruction sources の解決位置を
安定させるための中核属性である。依頼時の work_folder と Agent Profile の
`working_dir` の関係は Dispatch Review に表示する。
...
Windows の npm shim 経由では OutputContract を含む長い prompt が batch 引数として
壊れる場合があるため、`process.codex-cli` adapter は Windows では `codex.js` を
`node` で直接起動する。
...
を扱う。app-server の通知 transcript は AgentRun ExecutionRecord に保存する。

### 型定義

主要 Domain 型:

- `WorkItem`: id、title、description、acceptance_criteria、expected_artifacts、work_folder、constraints、locale、status、created_at、updated_at
- `AgentRun`: id、work_item_id、agent_profile_id、adapter、purpose、status、exit_code、execution_record_id、locale
- `AgentRunPurpose`: work、dispatch_preview、review、workflow_supervision
- `ResolvedSkillContext`: id、work_item_id、agent_profile_id、capability_probe_id、capabilities_in_force、instruction_sources、execution_record_uri、content_hash、locale
- `AgentOutputContract`: contract、instruction_pack、required、injection
- `AgentOutputContracts`: work、review、dispatch、supervision の purpose別 OutputContract
- `AgentOutputRecord`: AgentRun の最終出力を OutputContract に沿ってparseした結果。fields、questions、next_action、warnings、execution_record_id を持つ。質問に対する回答など、Agent の生成結果そのものはここに残す
- `ReviewResult`: review AgentOutputRecord から導出する評価結果。verdict、findings、requested_changes、referenced_artifacts、criteria_results を持ち、Work Item 状態遷移に接続する
...
- `WorkItemHistoryStep`: ledger の各recordから生成する統一 read model。request、dispatch、work、review、input、handoff、recovery、approval を同じ形に正規化し、artifact / evidence / agent output / workflow decision は facts / links / source_record_ids に畳み込んで表示する
- `WorkItemTimelineEvent`: 低レベル互換用 read model。新しいUIとCLI表示は `WorkItemHistoryStep` を優先する
- `ResolvedRunPacket`: id、work_item_id、agent_profile_id、adapter_id、purpose、working_dir、goal、path、work_folder、dispatch_plan_id、permission_policy_id、workspace_policy_id、resolved_skill_context_id、output_contract、review_checks、constraints、execution_record_uri、content_hash、locale
- `DispatchPlan`: id、work_item_id、status、agent_run_id、dispatch_agent_profile_id、target_agent_profile_id、resolved_run_packet_id、raw_output_execution_record_id、path、summary、risks、missing_information、selection_warnings、locale
- `RecoveryPlan`: id、work_item_id、status、action、target_agent_profile_id、failure_class、reason、summary、source_event_id、command_hint、prompt_hint、warnings、locale
- `WorkflowDecision`: id、work_item_id、action、source、reason、requires_human、target_agent_profile_id、agent_run_id、confidence、command_hint、warnings、locale
- `Artifact`: id、work_item_id、agent_run_id、artifact_type、uri、title、locale
- `ExecutionRecord`: id、work_item_id、agent_run_id、record_type、uri、title、locale
- `Evidence`: id、work_item_id、claim、basis、artifact_id、execution_record_id、produced_by、locale
- `HandoffPacket`: id、work_item_id、from_agent_profile、to_agent_profile、reason、summary、current_state、open_questions、artifact_ids、execution_record_ids、review_result_ids、next_request、locale
- `HumanDecision`: id、work_item_id、decision_type、rationale、locale
- `ScopeResolution`: path、work_folder、agent_profile_id、review_agent_profile_id、scope_match、permission_policy_id、workspace_policy_id、review_checks、warnings

Agent Management 型:

- `RuntimeDeclaration`
- `AdapterDeclaration`
- `AgentProfileDeclaration`: runtime、adapter、role、working_dir、description、specialties、output_contracts を持つ。role は `planner`、`worker`、`reviewer`、`dispatcher` などを許容し、domain は profile id / specialties / description で表す。
- `CapabilityProbe`
...
2. `nagare item preview` で Agent Profile、working_dir、CapabilityProbe、Policy、Review観点、Run Packet を確認する。
3. dispatch_agent は最大 5 件の compact な Agent Profile 候補から target Agent Profile を選び、DispatchPlan に記録する。候補には id、role、adapter、working_dir、specialties、description を含め、`code-planner`、`docs-worker`、`ui-reviewer` のような domain × role の選択を可能にする。
...
6. Adapter が stdout/stderr、Agent output、exit code を回収する。
7. Git work tree に差分があれば changed files / diff patch を ExecutionRecord として保存する。
8. 成功なら Work Item を `ready_for_review` にする。
9. Evidence と実行ログ ExecutionRecord を保存する。Work Item が期待成果物を持つ場合だけ、対応する成果物ファイルを Artifact として保存する。
10. Review が pass なら承認待ちに進む。CI/test/artifact check は Review 内で実行し、ReviewResult に残す。
11. `nagare decision approve` で `done` にする。

### Agent Run 失敗

1. Agent Run が non-zero exit、timeout、protocol violation、cancel のいずれかで終了する。
2. Work Item を `changes_requested` または `needs_human` にする。
3. 失敗ログを ExecutionRecord として保存する。
...
8. `nagare item advance --until-blocked true` は recovery 作成、handoff、人間入力、承認などの停止点で止まる。

### Handoff

1. 失敗またはレビュー結果から Handoff Packet を作成する。
2. `from_agent_profile`、`to_agent_profile`、reason、summary に加え、現在状態、未解決質問、artifact、diff、review、next request を保存する。
...
.nagare/artifacts/
  ユーザー依頼の成果物、または成果物作成に必要な入力ファイル

.nagare/logs/
  command log、adapter log、raw output、transcript、diff などの ExecutionRecord 本体
```
...
- Review checks は ReviewResult と ExecutionRecord に紐づく。
- Handoff は from/to の Agent Profile を必ず持つ。
- Run Packet は解決済み Skill Context、Policy、実行目的、作業ディレクトリ、依頼時の work_folder、実行目標の hash / id または値を持つ。
...
- Preview / Handoff Dispatch は DispatchPlan を ledger に保存し、raw output execution record を参照する。
...
- `item run` の agent 解決順は、明示 `--agent`、明示 accepted `--dispatch-plan`、最新 accepted DispatchPlan、`work_agent` とする。
...
- `worker`: worker / `process.codex-cli`
- `reviewer`: reviewer / `stdio.codex-app-server`
- `dispatcher`: dispatcher / `process.codex-cli`
- `supervisor`: supervisor / `process.codex-cli`
...
- `language`: `ja-JP` / `en-US` など。Nagare が生成する Evidence / Review / Decision の文言と記録に保存する
...
- Capability Probe: Agent が現在の working_dir で観測した capability / instruction source
...
- 整合性違反: passing review なしの approval、Agent working_dir と work_folder の不整合
...
- 解決失敗: Agent capability 不足、permission approval 不足
...
Permission Policy は Agent Profile / Capability Probe と分離する。
...
- review finding が approval をブロックする
...
nagare agent add --id <agent_profile_id> --runtime <runtime_id> --adapter <adapter_id> [--display-name <text>] [--working-dir <relative_path>] [--description <text>] [--specialties <csv>] [--root <path>]
nagare agent update <agent_profile_id> [--display-name <text>] [--working-dir <relative_path>] [--description <text>] [--specialties <csv>] [--root <path>]
...
nagare item create --title <title> [--description <text>] [--acceptance <csv>] [--artifact <csv>] [--work-folder <relative_path>] [--constraint <csv>] [--workflow-mode confirm_first|finish_first] [--root <path>]
...
nagare item preview <work_id> --work-folder <path> [--agent <agent_profile_id>]
nagare item run <work_id> --work-folder <path> [--agent <agent_profile_id>]
```
