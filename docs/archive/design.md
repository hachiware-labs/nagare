...
- HandoffPacket。
  - current_state、open_questions、artifact_ids、execution_record_ids、
...
- 新規作成は右上の `Create New Item` から専用画面へ移動する。
...
- Work Queueを主領域に置き、作成導線は右上の `Create New Item` に集約する。
...
- Work Queueから入った詳細であることをパンくずで示す。
...
- review passedかつ `verification_hint` があり verification missingの場合は `検証を実行`。
- verification failedの場合は `回復案を見る` をprimaryにし、再検証 / handoff / artifact確認はsecondaryにする。
- review passed後は `承認する` をprimaryにする。`verification_hint` がある場合だけ verification passed 後に `承認する` をprimaryにする。
- approveできない状態では `承認` をprimaryにしない。表示する場合はdisabledにし、
  「レビュー後に承認可」「検証後に承認可」のように理由を添える。

デザイン:

- 左ナビにDetail専用項目を置かず、本文上部に `Work Queue / Detail` を表示する。
...
- Timeline は `request -> workflow_decision -> dispatch -> run -> artifact/evidence -> review -> approval` を基本の流れにする。`verification_hint` がある Item では review と approval の間に verification を挟む。
...
- Project単位の workflow policy、Domain、Artifact Type、Agent Profile を扱う。

表示・操作:

- Settings は `Workflow`、`Domains`、`Agents` のタブで分ける。
  - 1つの設定領域だけを表示し、長い管理画面にしない。
  - 保存後は対象タブのURL hashへ戻り、作業文脈を維持する。
- Workflow: `default_progress_mode` と `approval_policy` を保存する。
- Domains: Domain と Artifact Type を1タブ内の2つの一覧で表示し、各一覧の直上に新規作成ボタンを置く。各行から編集・削除できるようにする。
  - Domain: 大きな業務領域、共通知識、共通Rubric、共通workflow default を編集する。
  - Artifact Type: `domain_id`、`description`、`artifact_types`、`rubric`、`dispatch_hints`、workflow override を編集する。
- Agents: `display_name`、`role`、`working_dir`、`description`、`specialties`、Domain / Artifact Type scope を編集する。
- fallback Agent 割り当ては通常の Settings UI には出さない。
...
- Domain / Artifact Type scope を選べる。
- project-local agent の add / update / delete を担う。

### 4.11 Agent Selection Policy

やること:

- Work / Review の担当は Settings で固定せず、dispatch が Agent Profile を見て選ぶ。
- Agent Profile の role、specialties、description、working_dir、domain_ids、artifact_type_ids を選択材料にする。
- Domain は大きな業務領域を束ね、共通知識、共通Rubric、dispatch_hints、workflow default を持つ。Domain は将来任意の拡張ではなく、ターゲット設計の第一級概念として扱う。
- Artifact Type は domain_id、description、artifact_types、rubric、dispatch_hints、workflow override を持つ。
- Rubric はAgent個別ではなくDomainに置き、Planner / Worker / Reviewer が目的に応じて同じ基準を使う。
- workflow は Work Item explicit、Domain override、Domain default、Project default の順に解決し、Item 作成時に解決済み `workflow_mode` と `approval_policy`、`domain_id`、`artifact_type_id` を保存する。
- Domain / Artifact Type / Rubric / Agent の関係図は `docs/domain-rubric-agent-design.html` を正とする。
- fallback 用の既定Agentは内部・高度設定として扱い、通常の Settings UI には出さない。
...
12. Settings / Agent Selection Policy。
...
Create New Item から完了までの詳細なユースケースは
[`create-new-item-usecase.md`](create-new-item-usecase.md) を参照する。
画像パターンの批判的評価は
[`create-new-item-ui-evaluation.md`](create-new-item-ui-evaluation.md) に記録する。
...
- Settings Domains（Domain / Artifact Type 2リスト）: [SVG](design-assets/svg/23-settings-domains.svg) / [PNG](design-assets/png/23-settings-domains.png)
- Agent Domain / Artifact Type scope Form: [SVG](design-assets/svg/24-agent-domain-scope-form.svg) / [PNG](design-assets/png/24-agent-domain-scope-form.png)
- Domain / Artifact Type / Rubric / Agent Design: [HTML](domain-rubric-agent-design.html)
...
- Create New Item Composer: [SVG](design-assets/svg/17-create-new-item-composer.svg) / [PNG](design-assets/png/17-create-new-item-composer.png)
- Item Created Dispatching: [SVG](design-assets/svg/18-item-created-dispatching.svg) / [PNG](design-assets/png/18-item-created-dispatching.png)
- Item Processing Running: [SVG](design-assets/svg/19-item-processing-running.svg) / [PNG](design-assets/png/19-item-processing-running.png)
- Item Approval Ready: [SVG](design-assets/svg/20-item-approval-ready.svg) / [PNG](design-assets/png/20-item-approval-ready.png)
- Item Done Summary: [SVG](design-assets/svg/21-item-done-summary.svg) / [PNG](design-assets/png/21-item-done-summary.png)
- State Display Patterns: [SVG](design-assets/svg/22-state-display-patterns.svg) / [PNG](design-assets/png/22-state-display-patterns.png)

画像生成元は `scripts/generate-ui-mockups.js` とする。PNGも更新する場合は
`NAGARE_RENDER_PNG=1 node scripts/generate-ui-mockups.js` を実行する。
