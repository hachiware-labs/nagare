...
そのため、UIの中心は常に Work Item とする。Dispatch、Run、Review、
Evidence、Verification、Handoff、Decision は、Work Item Detail の中で
因果関係として見せる。
...
- Review が approve / handoff / rerun の判断材料になる。
...
- Dispatch、Run、Review、Evidence、Verification、Handoff、Decisionを
  一続きの流れとして確認する。
- 追加指示、質問、Handoff、再実行も同じTimeline上のイベントとして確認する。
- 次に必要な操作を実行する。
...
- run failedの場合は `原因を見る` または `再実行` をprimaryにし、handoffはsecondaryにする。
- run succeededかつverification missingの場合は `検証を実行`。
- verification failedの場合は `再検証` をprimaryにし、handoff / artifact確認はsecondaryにする。
- verification passedの場合だけ `承認する` をprimaryにする。
...
- Timeline は `request -> dispatch -> run -> artifact/evidence -> verification -> review` を基本の流れにする。
- Handoff、人への質問、人の回答、追加指示、再実行は同じTimeline上のイベントとして追加する。
...
- Work Item Summary には current target、latest review verdict、open handoff、next action を表示する。
...
### 4.7 Handoff Inspector

やること:

- from agent、to agent、reason、summaryを入力・確認する。
- handoff packetに含める current state、artifact、open questionsを確認する。
- handoff dispatchを実行する。
- 作成されたDispatchPlanをDispatch Reviewへ接続する。

### 4.8 Agent Edit Modal
...
- work_agent、review_agent、dispatch_agentを選ぶ。
...
2. Work Queue。
3. Work Item Detail。
4. Dispatch Review Inspector。
5. Run Log Inspector / Artifact Viewer。
6. Review Inspector。
7. Evidence / Verification / Handoff Inspector強化。
8. Agent Profiles。
9. Agent Profile Detail。
10. Settings。
