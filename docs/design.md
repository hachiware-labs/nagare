...
- Project単位の workflow policy、Domain Group、Domain Profile、Agent Profile を扱う。
...
- Domains: Domain Group と Domain Profile を1タブ内の2つの一覧で表示し、各一覧の直上に新規作成ボタンを置く。各行から編集・削除できるようにする。
  - Domain Group: 大きな業務領域、共通知識、共通Rubric、共通workflow default を編集する。
  - Domain: `group_id`、`description`、`artifact_types`、`rubric`、`dispatch_hints`、workflow override を編集する。
- Agents: `display_name`、`role`、`working_dir`、`description`、`specialties`、Domain Group / Domain scope を編集する。
...
- Domain Group / Domain scope を選べる。
...
- Agent Profile の role、specialties、description、working_dir、domain_group_ids、domain_ids を選択材料にする。
- Domain Group は大きな業務領域を束ね、共通知識、共通Rubric、dispatch_hints、workflow default を持つ。Domain Group は将来任意の拡張ではなく、ターゲット設計の第一級概念として扱う。
- Domain Profile は group_id、description、artifact_types、rubric、dispatch_hints、workflow override を持つ。
- Rubric はAgent個別ではなくDomainに置き、Planner / Worker / Reviewer が目的に応じて同じ基準を使う。
- workflow は Work Item explicit、Domain override、Domain Group default、Project default の順に解決し、Item 作成時に解決済み `workflow_mode` と `approval_policy`、`domain_group_id`、`domain_id` を保存する。
- Domain / Rubric / Agent の関係図は `docs/domain-rubric-agent-design.html` を正とする。
...
- Settings Domains（Domain Group / Domain 2リスト）: [SVG](design-assets/svg/23-settings-domain-groups.svg) / [PNG](design-assets/png/23-settings-domain-groups.png)
- Agent Domain Scope Form: [SVG](design-assets/svg/24-agent-domain-scope-form.svg) / [PNG](design-assets/png/24-agent-domain-scope-form.png)
- Domain / Rubric / Agent Design: [HTML](domain-rubric-agent-design.html)
